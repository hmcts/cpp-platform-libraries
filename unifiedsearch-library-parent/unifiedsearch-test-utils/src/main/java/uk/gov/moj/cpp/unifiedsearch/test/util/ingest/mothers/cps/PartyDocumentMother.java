...
import static uk.gov.moj.cpp.unifiedsearch.test.util.ingest.mothers.cps.OffenceDocumentMother.defaultOffenceDocument;

import uk.gov.justice.services.unifiedsearch.client.constant.CpsPartyType;
...
public class PartyDocumentMother {
    ...
    public static PartyDocument.Builder defaultPartyAsBuilder() {
        ...
        builder.withPncId(randomPncId())
                .withOicShoulderNumber("111")
                .withAliases(AliasDocumentMother.randomAliasList());
        final String partyType = randomPartyType();
        builder.with_party_type(asList(partyType));
        if (CpsPartyType.DEFENDANT.name().equals(partyType)) {
            builder.withAsn("ASN " + ThreadLocalRandom.current().nextInt(1000, 9999));
    ...
    private static String randomPartyType() {
        ...
        return CpsPartyType.values()[index].name();
    }
