...
public class DefaultUnifiedSearchIndexerTest {
    ...
    public void shouldUnwrapTheDocumentAndPassToTheSearchIndexer() throws Exception {
        ...
        defaultUnifiedSearchIndexer.indexData(eventWithJoltTransformedPayload);

        final InOrder inOrder = inOrder(jsonValidator, searchIndexer);

        inOrder.verify(jsonValidator).validate(document, IndexInfo.CRIME_CASE.getIndexSchemaFile());
        inOrder.verify(searchIndexer).indexData(document, IndexInfo.CRIME_CASE.getIndexName());
    }
}
